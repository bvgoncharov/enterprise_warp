...
from .enterprise_warp import get_noise_dict
...
class StandardModels(object):
  ...
  def measurement_noise(self, option={}):
    ...
    if self.params.efac < 0 and self.params.equad < 0:
      noisedict = get_noise_dict(psrlist=[p.name for p in self.params.psrs], noisefiles=self.params.noisefiles)
    elif self.params.efac >= 0 or self.params.equad < 0:
      raise ValueError('Both EFAC and EQUAD must be fixed (-1) for measurement_noise')
    else:
      noisedict = {}
    
    measurement_noise_values = ds.makenoise_measurement(self.psr, noisedict=noisedict, selection=se)
  ...
  def equad(self,option={}):
    ...
    if option["selection"] != "by_backend": # not in selections.__dict__.keys():
      ...
      se = ds.signals.selection_backend_flags
    if self.params.equad < 0:
      noisedict = get_noise_dict(psrlist=[p.name for p in self.params.psrs], noisefiles=self.params.noisefiles)
    eqs = ds.makenoise_measurement(self.psr, noisedict=noisedict, selection=se)
    return eqs
  ...
  def ecorr(self,option={}):
    ...
    if option["selection"] != "by_backend": # not in selections.__dict__.keys():
      ...
      se = ds.signals.selection_backend_flags
    if self.params.ecorr < 0:
      noisedict = get_noise_dict(psrlist=[p.name for p in self.params.psrs], noisefiles=self.params.noisefiles)
    ecs = ds.makegp_ecorr(self.psr, noisedict=noisedict, enterprise=False, scale=1.0, selection=se, name='ecorrGP')
    return ecs
