...
* **{x} (a number in curly brackets)**: a separator, indicating that following parameters are only for model 'x'. If we specify more than one model, then Enterprise is launched in model comparison regime using `class HyperModel` from [enterprise_extensions](https://github.com/stevertaylor/enterprise_extensions/).
